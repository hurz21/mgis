...
Module wrapItextSharp
    ...
    Friend Function createImagePdf(pngdatei As String, outPDF As String, w As Integer, h As Integer, isa4checked As Boolean) As Boolean
        Try
            ...
            If isa4checked Then
                _aFormat = m.dina4InMM
                post_PDF_FORMAT = "A4"
            Else
                _aFormat = m.dina3InMM
                post_PDF_FORMAT = "A3"
            End If
    ...
    Private Sub PDF_format_festlegen(ByRef A4dim As clsCanvas, kuerzel As String)
        Select Case kuerzel
            Case "A3"
                A4dim.w = m.dina3InPixel.h
                A4dim.h = m.dina3InPixel.w
            Case "A4"
                A4dim.w = m.dina4InPixel.h
                A4dim.h = m.dina4InPixel.w
            Case Else
                A4dim.w = m.dina4InPixel.h
                A4dim.h = m.dina4InPixel.w
        End Select
    ...
    Friend Sub createText2PDF(text As String, ByRef ausgabedatei As String)
        l("createText2PDF---------------------- anfang")
        ...
        Try
            ...
            addChunk2Dok(oDoc, "GIS-Dossier " & " ", bf, 14, zeilenabstand_gross)
            addChunk2Dok(oDoc, " " & " ", bf, 14, zeilenabstand_gross)

            addChunk2Dok(oDoc, "(- Vorabversion, wird später ansehnlicher - )", bf, 8, zeilenabstand_gross)
            addChunk2Dok(oDoc, " " & " ", bf, 14, zeilenabstand_gross)
            addChunk2Dok(oDoc, " " & " ", bf, 14, zeilenabstand_gross)
            Dim temp As String = ""
            ...
            'cb.EndText()
        Catch ex As Exception
            l("FEhler in createText2PDF: " & ex.ToString)
        Finally
            oDoc.Close()
            Writer.Close()
            l("createText2PDF Ende")
        End Try
    End Sub
