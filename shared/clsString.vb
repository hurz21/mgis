...
ende:
    ...
    Friend Shared Function removeLastChar(text As String) As String
        Dim a As String
        Try
            l("removeLastChar---------------------- anfang")
            a = Substring(text, 0, text.Length - 1)
            Return a
            l("removeLastChar---------------------- ende")
        Catch ex As Exception
            l("Fehler in removeLastChar: ", ex)
            Return text
        End Try
    End Function
    ...
    '		Resume Next
    '	End Function
    Public Shared Function ue2umlaut(ByVal s As String) As String ' von byref auf byval umgestellt
        Try
...
#End Region
    ...
    Shared Function date2string(ddd As Date, modus As Integer) As String
        Try
            Select Case modus
                ...
                Case 2
                    Return ddd.ToString("yyyyMMdd_HHmmss").Trim 'Format(Now, "yyyyMMdd_hhmmss") 
                Case 3
                    Return ddd.ToString("yyyyMMdd").Trim
                Case 4
                    Return ddd.ToString("dd.MM.yyyy").Trim
                Case 5
                    Return Format(ddd, "yyyyMMddhhmmssFFF")
            End Select
            Return "???"
        Catch ex As Exception
            Return "???"
    ...
    'Friend Shared Function isInIntArray(gids() As Integer, v As Integer) As Boolean
    '    Try
    '        For Each iii In gids
    '            If v = iii Then Return True
    '        Next
    '        Return False
    '    Catch ex As Exception
    '        Return False
