...
#If DEBUG Then
        getMapsFromInternet = False
        iminternet = False
        HauptServerName = "w2gis02"
#Else
        HauptServerName = "w2gis02"
        iminternet = False
#End If
...
#End If
        ...
        userIniProfile = New clsINIDatei(mgisRemoteUserRoot & "userinis\" & GisUser.username & ".ini")
    ...
    Private Shared Sub defineDinA4Dina3Formate()
...
#End If
        ...
        clsGisstartPolitik.registerAutostart("mgisAktualisieren.bat", myglobalz.mgisRemoteUserRoot)
        ...
        clsINIDatei.UserinifileAnlegen(0, mgisRemoteUserRoot & "userinis\", GisUser.username)
...
#End If
    ...
    Private Sub zweitenBildschirm()
        ...
        If aufzweitembildschirmstarten Then
            ...
            Else
                'Dim test As Double = -1 * SystemParameters.PrimaryScreenWidth
                'If test < 0 Then
                '    Left = 0
                'Else
                '    Left = test
    ...
    Private Sub refreshMap(vgrundRefresh As Boolean, hgrundrefresh As Boolean)
        ...
        Debug.Print(Width & "," & Height & "," & System.Windows.SystemParameters.PrimaryScreenWidth)
        'initMassstab(cvtop.Width, cvtop.Height, (System.Windows.SystemParameters.PrimaryScreenWidth - dockMenu.Width))
    ...
    Sub presentMap(vgrundRefresh As Boolean, hgrundrefresh As Boolean)
        ...
        Catch ex As Exception
            l("fehler in presentMap: " & countererror & ex.ToString)
        End Try
    End Sub
    ...
    Private Function genImageMapTextstring() As String
        ...
        If CDbl(layerActive.masstab_imap) > aktMasstab Then
            'isok
            imgKarte.Width = cvtop.Width
            imgKarte.Height = cvtop.Height
...
#End If
    ...
    Private Sub setCanvasSizes(sizecanvas As Canvas)
        ...
        imageMapCanvas.Width = CLng(sizecanvas.Width * faktor)
        imageMapCanvas.Height = CLng(sizecanvas.Height * faktor)
        ...
        MainListBox.Height = sizecanvas.Height - spMenueHead.Height
        'spMenueHead.Height
    End Sub
    ...
    Sub MapModeAbschicken(aslot As clsSlot)
        Try
            ...
            If aslot.slotnr = 80 Or aslot.slotnr = 81 Or aslot.slotnr = 82 Then
                aslot.image.Source = aslot.bitmap
            Else
                'aslot.image.Source = aslot.bitmap 
    ...
    Private Function slotImageDownloadFailed(slotnr As Integer) As EventHandler(Of ExceptionEventArgs)
        Dim info As String = "Eine " & slots(slotnr).funktion & " '" & slots(slotnr).layer.titel & "' konnte nicht erstellt werden (Timeout).  " & Environment.NewLine & Environment.NewLine
...
#Enable Warning BC42025 ' Access of shared member, constant member, enum member or nested type through an instance
    ...
    Private Sub canvas1_MouseLeftButtonUp(ByVal sender As Object, ByVal e As System.Windows.Input.MouseButtonEventArgs) Handles cvtop.MouseLeftButtonUp, cvtop.MouseLeftButtonUp
        ...
        Select Case CanvasClickModus.ToLower
            ...
            Case "wmsdatenabfrage"
                ...
                Dim url As String
                url = clsWMS.calcWMSGetfeatureInfoURL(bbox, layerActive.aid, CInt(cvtop.Height), CInt(cvtop.Width),
    ...
    Private Sub adresssuche()
        ...
        Debug.Print(adrs.strasseOhneHausnr.ToString)
        If ergebnis Then
            If adrs.strasseOhneHausnr Then
                Debug.Print(adrs.strasseOhneHausnr.ToString & aktadr.ToString)
                OSrefresh = True
                os_tabelledef.Schema = "public"
                os_tabelledef.tabelle = "haloschneise"
                os_tabelledef.gid = CType(getGID4Street(aktadr, os_tabelledef.Schema, os_tabelledef.tabelle), String)
                'rangeholen
                Dim box As String = ""
                Dim xl, xh, yl, yh As Double

                l("rechtsHochwertHolen---------------------- anfang")
                box = pgisTools.holeBoxKoordinatenFuerStrasse(os_tabelledef.gid, os_tabelledef.Schema & "." & os_tabelledef.tabelle)
                ...
                Else
                    ...
                    If postgisBOX2range(box, xl, xh, yl, yh) Then
                        kartengen.aktMap.aktrange.xl = xl - puffer
                        kartengen.aktMap.aktrange.xh = xh + puffer
                        kartengen.aktMap.aktrange.yl = yl - puffer
                        kartengen.aktMap.aktrange.yh = yh + puffer
                        aktGlobPoint.X = kartengen.aktMap.aktrange.xcenter
                        aktGlobPoint.Y = kartengen.aktMap.aktrange.ycenter
                    Else
    ...
    Private Sub google3dintro()
        ...
        Try
            ...
            End If
            gis = Nothing
            ' Protokollausgabe_aller_Zugriff("ja")6
    ...
    Private Sub btnVogel_Click(sender As Object, e As RoutedEventArgs)
        ...
        webBrowserControlVogel.Navigate(New Uri(uncdatei))
        'Process.Start(clsStartup.calcURI4vogel)
        e.Handled = True
    End Sub
    ...
    Private Sub chkauswahlgeaendert(sender As Object, e As RoutedEventArgs)
        ...
        Else
            ...
            For Each lay As clsLayerPres In layersSelected
                If lay.aid <> CInt(CStr(nck.Tag)) Then
                    Continue For
                End If
                lay.isactive = False
        ...
        Debug.Print("action " & action & " aid: " & CStr(nck.Tag))
        If action = "sub" Then
            For i = 0 To slots.Length - 1
                If slots(i).layer.aid = CInt(CStr(nck.Tag)) Then
                    slots(i).setEmpty()
                    slots(i).layer.clear()
                    slots(i).mapfile = ""
                    Exit For
...
#End If
    ...
    '    'cgi = cgi & " -m \\ myglobalz.HauptServerName \gdvell\websys\mapfiles\cache\feinen_j.map "
    '    cgi = cgi & mapfileBILD
    '    cgi = cgi & " -e 480445 5541797 481229 5542316 -s 1658 1097  "
    '    cgi = cgi & " -o \\myglobalz.HauptServerName \gdvell\cache\gis\feinen_j_lubtest.png  "
    ...
    Private Shared Sub setMyPdfRectA3(hochQuerModus As String, altesRechteck As Rectangle)
        Dim w = altesRechteck.Width
        Dim h = altesRechteck.Height
    ...
    Private Sub btnPNG_Click(sender As Object, e As RoutedEventArgs)
        ...
        makeandloadPDF("mitmasstab", PDF_PrintRange, PDF_druckMassStab, ausrichtung, tbPDF_Bemerkung.Text, tbPDF_Ort.Text, True, hochaufloesend,
                       CBool(rbFormatA4.IsChecked), False, ausgabedatei, CBool(cbmitsuchobjekt.IsChecked))
...
#Enable Warning BC42025 ' Access of shared member, constant member, enum member or nested type through an instance
    ...
    Private Sub PDFmitmasstab(ByRef ausgabedatei As String)
        ...
        makeandloadPDF("mitmasstab", PDF_PrintRange, CDbl(tbMasstabDruck.Text), ausrichtung, tbPDF_Bemerkung.Text, tbPDF_Ort.Text, False,
                       hochaufloesend, CBool(rbFormatA4.IsChecked), False, ausgabedatei, CBool(cbmitsuchobjekt.IsChecked))
    End Sub
    ...
    Private Sub PDFohneMasstab(schnelldruck As Boolean, ByRef ausgabedatei As String)
        ...
        makeandloadPDF("ohnemasstab", PDF_PrintRange, PDF_druckMassStab, ausrichtung, tbPDF_Bemerkung.Text, tbPDF_Ort.Text,
                       False, hochaufloesend, CBool(rbFormatA4.IsChecked), schnelldruck, ausgabedatei, CBool(cbmitsuchobjekt.IsChecked))
    End Sub
    ...
    Sub resizeWindow()
        ...
        If Height < 800 Then
            Height = 800
        End If
        'dockTop.Height = 50 wird nur bei start initialisiert
    ...
    Private Sub slotsResize(width As Double, height As Double)
        Try
            ...
            l(" slotsResize ---------------------- ende")
        Catch ex As Exception
            l("Fehler in slotsResize: " & ex.ToString())
        End Try
    End Sub
    ...
    Private Sub Window_StateChanged(sender As Object, e As EventArgs)
        ...
        Select Case WindowState
            Case WindowState.Maximized
                ...
                'cvtop.VerticalAlignment = VerticalAlignment.Top
