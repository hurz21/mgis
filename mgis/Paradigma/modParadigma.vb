...
Module modParadigma
    Friend Function getuserlayeraid(userName As String) As Integer
        Try
            ...
            Dim dt As DataTable
            dt = getDTFromWebgisDB("select userlayeraid  from public.nutzer where lower(name)='" & userName.Trim.ToLower & "'", "webgiscontrol")
            ' webgisREC.mydb.SQL = "select userlayeraid  from public.nutzer where lower(name)='" & userName.Trim.ToLower & "'"
    ...
    Private Function darfVorgangBearbeiten(userName As String, aktvorgangsid As String) As Boolean
        ...
        Try
            ...
            Dim dt As DataTable
            Dim sql As String = "select bearbeiter,weiterebearb from stammdaten where vorgangsid=" & aktvorgangsid
    ...
    Function UsernamePasstZuBearbeiterString(user As String, bearbstring As String) As Boolean
        Try
            ...
            sql = "select initial_ from bearbeiter where lower(username)='" & user.ToLower & "'"
            dt = modgetdt4sql.getDT4Query(sql, paradigmaMsql, hinweis)
            ' dt = modParadigma.getDTFromParadigmaDB(, paradigmaDBTyp)
            ...
            End If
        Catch ex As Exception
            nachricht("fehler in UsernamePasstZuBearbeiterString:" & ex.ToString)
            Return False
        End Try
    End Function
    ...
    Friend Function GeometrieNachParadigma(aktPolygon As clsParapolygon, aktPolyline As clsParapolyline) As Boolean
        ...
        Try
            ...
            If modParadigma.darfVorgangBearbeiten(GisUser.username, aktvorgangsid) Then
    ...
    Private Function PolygonObjVorbereiten(aktPolygon As clsParapolygon) As clsParapolygon
        Try
            If aktPolygon.Typ = RaumbezugsTyp.Polygon Then
                ...
                aktPolygon.box.BBOX = holeKoordinatenFuerUmkreis(aktPolygon.ShapeSerial)
            ...
            Return aktPolygon
        Catch ex As Exception
            nachricht("Fehler in PolygonObjVorbereiten: " & ex.ToString)
            Return Nothing
        End Try
    End Function

    Friend Function deleteRaumbezug2all(rid As Integer, vid As Integer, v As String) As Boolean
        modSQLsTools.deleteRaumbezug2allOraclesqls(rid, vid, "raumbezug2vorgang")
        'If paradigmaDBTyp = "oracle" Then
        '    clsParadigmaDBOracle.deleteRaumbezug2allOracle(rid, vid, "raumbezug2vorgang")
        'End If
        'If paradigmaDBTyp = "sqls" Then
        '    modSQLsTools.deleteRaumbezug2allOraclesqls(rid, vid, "raumbezug2vorgang")
        'End If
    ...
    Friend Function deleteRaumbezug(rid As Integer, vid As Integer) As Boolean
        modSQLsTools.deleteRaumbezugSqls(rid, vid)
        'If paradigmaDBTyp = "oracle" Then
        '    clsParadigmaDBOracle.deleteRaumbezugOracle(rid, vid)
        'End If
        'If paradigmaDBTyp = "sqls" Then
        '    modSQLsTools.deleteRaumbezugSqls(rid, vid)
        'End If
    ...
    Function DokNachParadigma(dokFullname As String, vid As String, Beschreibung As String) As Boolean
        ...
        Try
            ...
            Dim sql, hinweis As String
            sql = "select arcdir from stammdaten where vorgangsid=" & vid
            'dt = modParadigma.getDTFromParadigmaDB(, paradigmaDBTyp)
            dt = modgetdt4sql.getDT4Query(sql, paradigmaMsql, hinweis)

            PARADIGMA_Vorgangs_ArchivSubdir = clsDBtools.fieldvalue(dt.Rows(0).Item(0))
            erzeugeUnterverzeichnis(PARADIGMA_ARCHIV_rootdir & PARADIGMA_Vorgangs_ArchivSubdir)
    ...
    Function getParadigmaAbteilung4FDumwelt(user As String) As String
        ...
        Try
            ...
            Return ParadigmaAbteilung
        Catch ex As Exception
            l("fehler in getAbteilung4FDumwelt ", ex)
            Return ""
        End Try
    End Function
    ...
    Friend Function getDTFromWebgisDB(queryString As String, Db As String) As DataTable
        l("getDTFromWebgisDB-------------------------")
        l(" webgisREC.mydb.Schema " & webgisREC.mydb.Schema)
