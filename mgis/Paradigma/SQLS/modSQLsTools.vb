...
Module modSQLsTools
    ...
    Private Sub populateDokumente(relativpfad As String, beschreibung As String, originalFullname As String, originalName As String, dateidatum As Date, vid As String, eid As Integer, newsavemode As Boolean, fo As FileInfo, revisionssicher As Boolean, extension As String)
        ...
        clsSqlparam.paramListe.Add(New clsSqlparam("INITIAL_", getInitial(GisUser.username.ToLower)))
    ...
    'Friend Function checkin_Dokumentesqls(relativpfad As String, beschreibung As String, originalFullname As String,
    '                                      originalName As String, dateidatum As Date, vid As String, eid As Integer,
    '                                      newsavemode As Boolean) As Integer
    '    Dim anzahlTreffer& = 0
    ...
    '    Dim fo As New IO.FileInfo(originalFullname)
    '    Dim pradigmaDB As New clsEigentuemerSQLS
    ...
    '        pradigmaDB.oeffneConnectionSQLS()
    '        com = New SqlCommand(querie, pradigmaDB.EigentuemerSchnellDB)
    '        nachricht("vor setParams  ")
    '        'seteFiledatum(fi, dateidatum)
    '        dateidatum = Now
    '        setSQLParamsDOKSQLS(com, relativpfad, fo, beschreibung, originalFullname, originalName, fo, False, dateidatum, CInt(aktvorgangsid),
    '                            eid, newsavemode)
    ...
    '        pradigmaDB.schliesseConnectionSQLS()
    '        fo = Nothing
    '        Return CInt(newid)
    '    Catch ex As Exception
    '        nachricht("Fehler Dok1  beim Abspeichern: " & vbCrLf & ex.ToString)
    ...
    Private Sub setSQLParamsDOKSQLS(com As SqlCommand, relativpfad As String, fo As FileInfo, beschreibung As String,
        ...
        com.Parameters.AddWithValue("@INITIAL_", getInitial(GisUser.username.ToLower))
    ...
    '        com = New SqlCommand(querie, pradigmaDB.EigentuemerSchnellDB)
    '        nachricht("vor setParams  ")
    '        setSQLParamsRBSQLS(com, aktrb, 0)
    '        Dim kobjssss = com.ExecuteScalar()
    '        If kobjssss Is Nothing Then
    '            newid = 0
    '        Else
    ...
    Private Sub populateParaadresse()
        ...
        Catch ex As Exception
            nachricht("Fehler beim populateParaadresse: " & ex.ToString)
        End Try
    End Sub
    ...
    '        com = New SqlCommand(querie, pradigmaDB.EigentuemerSchnellDB)
    '        nachricht("vor setParams  ")
    '        setSQLParamsFLST_serial(com, vid, rid, serial, 0, typ, area)
    '        Dim kobjssss = com.ExecuteScalar()
    '        If kobjssss Is Nothing Then
    '            newid = 0
    '        Else
    ...
    Friend Sub deleteRaumbezugSqls(rid As Integer, vid As Integer)
        Dim anzahlTreffer& = 0, hinweis$ = "", newid& = 0
        Dim com As SqlCommand
        Dim querie As String
    ...
    '    Dim pradigmaDB As New clsEigentuemerSQLS
    '    Try
    '        querie = "delete from raumbezug where RAUMBEZUGSID=@RAUMBEZUGSID"
    '        nachricht("nach setSQLbody : " & querie)
    '        querie = querie & ";SELECT CAST(scope_identity() AS int);"
    '        nachricht("nach setSQLbody : " & querie)
    '        pradigmaDB.oeffneConnectionSQLS()
    ...
    Friend Sub deleteRaumbezug2allOraclesqls(rid As Integer, vid As Integer,
                                             tablename As String)
        Dim anzahlTreffer& = 0, hinweis$ = "", newid& = 0
        Dim com As SqlCommand
        Dim querie As String
    ...
    '    Dim pradigmaDB As New clsEigentuemerSQLS
    '    Try
    '        Dim querie As String = "INSERT INTO ParaFlurstueck (GEMCODE,FLUR,ZAEHLER,NENNER,ZNKOMBI,GEMARKUNGSTEXT,FS,FLAECHEQM) " +
    '                        " VALUES (@GEMCODE,@FLUR,@ZAEHLER,@NENNER,@ZNKOMBI,@GEMARKUNGSTEXT,@FS,@FLAECHEQM)"
    '        querie = querie & ";SELECT CAST(scope_identity() AS int);"
    '        nachricht("vor setParams  ")
    '        pradigmaDB.oeffneConnectionSQLS()
    '        com = New SqlCommand(querie, pradigmaDB.EigentuemerSchnellDB)
    '        SETSQLPARAMSFLSTsqls(com)
    '        Dim kobjssss = com.ExecuteScalar()
    '        If kobjssss Is Nothing Then
    '            newid = 0
    '        Else
    ...
    '        nachricht("vor setParams  ")
    '        pradigmaDB.oeffneConnectionSQLS()
    '        com = New SqlCommand(SQLupdate, pradigmaDB.EigentuemerSchnellDB)
