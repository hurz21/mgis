Public Class clsBSE
    Friend Shared Function getInfo4point(winpt As myPoint, item As clsDossierItem) As Boolean
        Dim resulttext As String = ""
        Try
            l(" getInfo4point ---------------------- anfang: " & item.schematabelle)
            Return getExtracted(item, winpt)
        ...
        End Try
    End Function
    Private Shared Function getExtracted(item As clsDossierItem, winpt As myPoint) As Boolean
        Dim dt As System.Data.DataTable
        l("getBaulastenExtracted ---------------------- " & item.schematabelle)
        item.kurz = "" : item.datei = ""
        dt = clsDossier.getDtHauptabfrageFlaeche(winpt, item.schematabelle)
        Try
    ...
    Private Shared Function bildeINFO(DataRow As System.Data.DataTable) As String
        Try
            ...
            For i = 0 To DataRow.Rows.Count - 1
                ...
                summe = summe & "Änderung veröffentlicht: " & clsDBtools.fieldvalue(DataRow.Rows(i).Item("veroeff_geaendert").ToString.Trim & trenn)
