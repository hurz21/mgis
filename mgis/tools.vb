...
Module mtools
    ...
    Sub setzeGrundFuerEigentuemerabfrage(ByRef text As String)
        If GrundFuerEigentuemerabfrage.IsNothingOrEmpty Then
            If STARTUP_mgismodus = "paradigma" Then
                GrundFuerEigentuemerabfrage = "FD 67, " & GisUser.username & ", " & aktvorgangsid
            Else
                If STARTUP_mgismodus = "probaug" Then
                    GrundFuerEigentuemerabfrage = "FD 63, " & GisUser.username & ", " & aktvorgang.az
                Else
                    GrundFuerEigentuemerabfrage = GisUser.username & ", " & aktvorgangsid & ", " & GisUser.ADgruppenname
                End If
            End If
        End If
    ...
    Friend Function zugrifferlaubt(userName As String) As Boolean
        ...
        '
        clsActiveDir.getall("mueller_b") '  GisUser.username)
    ...
    Friend Sub setUserFDkurz(gisuser As clsUser)
        l("setUserFDkurz userName " & gisuser.username)
        gisuser.username = gisuser.username.Trim
        'If binZuhause Then
        '      gisuser.favogruppekurz = "umwelt"
        '    clsActiveDir.fdkurz = "umwelt"
        '    Exit Sub
        'End If
        If clsActiveDir.getall(gisuser.username) Then
            If NSfstmysql.ADtools.istUserAlbBerechtigt(gisuser.username) Then
            ...
            End Select
        Else
            MessageBox.Show("Der User >" & gisuser.username & "< kommt im Active Directory  nicht vor!!!")
            l("fehler in setUserFDkurz Der User kommt im Active Directory nicht vor!!! " & gisuser.username)
            gisuser.favogruppekurz = "intranet"
        End If
