Public Class winBMedit
    ...
    Private Sub btnBookmarkSave_Click(sender As Object, e As RoutedEventArgs)
        ...
        Dim newid As Long
        If _modus = "neu" Then
            newbookmark = bmTools.bildebookMark(tbBMneu.Text, CBool(cbFreigabefuerGruppe.IsChecked))
            Dim erfolg As Boolean
            erfolg = bmTools.BMSaveInsert(newbookmark, newid)
            If erfolg Then
                MsgBox("Bookmark >" & newbookmark.titel & "< wurde mit der id: " & newid & " in den Bestand aufgenommen!")
            Else
                MsgBox("Es gab ein Problem, Version " & mgisVersion)
            End If
        End If
        If _modus = "edit" Then
            ...
            If erfolg Then
                MsgBox("Bookmark mit id>" & auswahlBookmark.id & "< wurde geändert!")
            Else
